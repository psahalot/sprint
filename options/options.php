...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Options', 'sprint');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'sprint');
		...
		$defaults['footer_credit'] = __('', 'sprint');
		
		$defaults['help_tabs'] = array();
		$defaults['help_sidebar'] = __('', 'sprint');
	...
	function _options_page_html(){
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<a href="http://ideaboxthemes.com" id="optionpanellogo" class="logo" target="_blank"><img src="'.$this->url.'img/optionpanellogo.png" /></a>';
echo '<span class="headtext">Welcome to your theme\'s mission control center.</span>';

					echo '<a href="http://ideaboxthemes.com/support" class="docsupport" target="_blank"><img src="'.$this->url.'img/support.png" />Support</a>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.__('<strong>Settings Imported!</strong>', 'sprint').'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.__('<strong>Settings Saved!</strong>', 'sprint').'</div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn">'.__('<strong>Settings have changed!, you should save them!</strong>', 'sprint').'</div>';
					echo '<div id="nhp-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'sprint').'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'sprint').'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/glyphicons/download.png" /><br/> '.__('Import / Export', 'sprint').'</a>';
							echo '</li>';
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> '.__('Dev Mode Info', 'sprint').'</a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Import / Export Options', 'sprint').'</h3>';
							
							echo '<h4>'.__('Import Options', 'sprint').'</h4>';
							...
							echo '<div id="nhp-opts-import-code-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Input your backup code below and hit Import to restore your sites options from a backup.', 'sprint')).'</p>';
							...
							echo '<div id="nhp-opts-import-link-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('nhp-opts-import-link-description',__('Input the URL to another sites options set and hit Import to load the options from that site.', 'sprint')).'</p>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Dev Mode Info', 'sprint').'</h3>';
							echo '<div class="nhp-opts-section-desc">';
				...
				echo '<div id="nhp-opts-footer">';
					...
					echo '<input type="submit" name="'.$this->args['opt_name'].'[defaults]" value="'.__('Reset to Defaults', 'sprint').'" class="button-secondary" />';
					echo '<input type="submit" name="submit" value="'.__('Save Changes', 'sprint').'" class="button-primary" />';
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
