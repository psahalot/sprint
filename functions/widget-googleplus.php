...
/*-----------------------------------------------------------------------------------
	...
	Plugin Name: IdeaBox Google Plus Badge Box
	Description: A widget for displaying Google Plus Badge Box.
	Version: 1.0

-----------------------------------------------------------------------------------*/
add_action('widgets_init', 'sprint_googleplus_load_widgets');

function sprint_googleplus_load_widgets()
{
	register_widget('sprint_googleplus_Widget');
}

class sprint_googleplus_Widget extends WP_Widget {
	
	function sprint_googleplus_Widget()
	{
		$widget_ops = array('classname' => 'sprint_googleplus', 'description' => __('Add Google Plus Badge Box.','sprint'));

		$control_ops = array('id_base' => 'google-badge-box');

		$this->WP_Widget('google-badge-box', __('IdeaBox: Google Plus Badge Box','sprint'), $widget_ops, $control_ops);
	}
...
<script type="text/javascript">
  (function() {
	...
	{
		$defaults = array('title' => __('Find us on Google Plus','sprint'), 'page_url' => '', 'width' => '300', 'color_scheme' => 'light', 'gp_layout' => 'portrait', 'page_type' => 'profile', 'cover_photo' => 'on', 'tagline' => 'on');
		$instance = wp_parse_args((array) $instance, $defaults); ?>
		
		<p>
			<label for="<?php echo $this->get_field_id('page_type'); ?>"><?php _e('Page type','sprint'); ?>:</label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Google+ Page URL','sprint'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width','sprint'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme','sprint'); ?>:</label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('gp_layout'); ?>"><?php _e('Layout','sprint'); ?>:</label> 
		...
		<p>
			<b><?php _e('Portrait Layout Settings','sprint'); ?></b>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('cover_photo'); ?>"><?php _e('Cover Photo','sprint'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('tagline'); ?>"><?php _e('Tagline','sprint'); ?></label>
		</p>
	<?php
	}
}
